...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    model.train()
    criterion.train()
    tensor_type = torch.cuda.HalfTensor if fp16 else torch.cuda.FloatTensor#16位float
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value:.6e}'))
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        samples.tensors = samples.tensors.type(tensor_type)#统一为16位float
        samples.mask = samples.mask.type(tensor_type)

        with torch.cuda.amp.autocast(enabled=fp16):#在矩阵相乘、linear、卷积等操作时自动转换为16位halffloat数据类型
            '''
             当进入autocast的上下文后，上面列出来的那些CUDA ops 会把tensor的dtype转换为半精度浮点型，从而在不损失训练精度的情况下加快运算。
             刚进入autocast的上下文时，tensor可以是任何类型，你不要在model或者input上手工调用.half() ，框架会自动做，这也是自动混合精度中“自动”一词的由来。
        ...
        更新权重：
        猜测：
        autocast会在需要某些计算时把数据调成16位，复制一份32位权重，生成16位梯度。
        unscale_或step会把16位梯度转化成32位，再unscale。scale后可能出现nan，则unscale后还有nan，step不更新，并在scaler.update减小scale系数
        对于要更新的权重，scaler.backward计算的16位梯度会更新到模型定义的32位权重中，后续计算用的就是新的32位权重了
        '''
...
#     metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        if tracker is not None:
            ...
            frame_id = targets[0].get("frame_id", None)#如果注释字典中没有frame_id的key，则返回None
        ...
        #if int(targets[0]["image_id"]) <= 1500:
         #   continue
        ...
        with torch.cuda.amp.autocast(enabled=fp16):
            if det_val:
                ...
                outputs, pre_embed = model(samples, pre_embed)#第一帧pre_embed是None，第二帧是{'reference': 第一帧的预测bbox, 'tgt': 第一帧的encoder最后的tgt, 'feat': 第一帧的backbone feature}
