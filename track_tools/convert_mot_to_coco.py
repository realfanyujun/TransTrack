...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        out_path = os.path.join(OUT_PATH, '{}.json'.format(split))#mot/test.json
        ...
        ann_cnt = 0
        video_cnt = 0
        for seq in sorted(seqs):#MOT20-04
            if '.DS_Store' in seq:
                continue
            if 'mot' in DATA_PATH and (split != 'test' and not ('FRCNN' in seq)):
                continue
            video_cnt += 1  # video sequence number.
            out['videos'].append({'id': video_cnt, 'file_name': seq})#videos: [{id: 1, file_name: MOT20-04}]
            seq_path = os.path.join(data_path, seq)#mot/test/MOT20-04
            img_path = os.path.join(seq_path, 'img1')#mot/test/MOT20-04/img1
            ann_path = os.path.join(seq_path, 'gt/gt.txt')#mot/test/MOT20-04/gt/gt.txt
            images = os.listdir(img_path)
            num_images = len([image for image in images if 'jpg' in image])  # half and half

            if HALF_VIDEO and ('half' in split):
                image_range = [0, num_images // 2] if 'train' in split else \
                              [num_images // 2 + 1, num_images - 1]#train_half，train的前一半；val_half，train的后一半
            else:
            ...
            for i in range(num_images):
                ...
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq, i + 1),  # image name.
                              'id': image_cnt + i + 1,  # image number in the entire training set. starting from 1
                              'frame_id': i + 1 - image_range[0],  # image number in the video sequence, starting from 1.
                              'prev_image_id': image_cnt + i if i > 0 else -1,  # image number in the entire training set.
                              'next_image_id': image_cnt + i + 2 if i < num_images - 1 else -1,
                              'video_id': video_cnt,
                              'height': height, 'width': width}
                out['images'].append(image_info)#images: [{}]将每个视频的图片依次加入数据集中形成索引
            ...
            if split != 'test':
                ...
                if CREATE_SPLITTED_ANN and ('half' in split):
                    ...
                    anns_out[:, 0] -= image_range[0]#frameid从0开始
