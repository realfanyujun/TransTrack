...
string.""", formatter_class=argparse.RawTextHelpFormatter)
    ...
    parser.add_argument('--groundtruths', type=str, default='/mnt/data2/UAVDT/GT/val/', help='Directory containing ground truth files.')
    parser.add_argument('--tests', type=str, default='/home/fan/Documents/GeanyProject/UAVTrack/output/val/tracks/', help='Directory containing tracker result files')
    ...
    parser.add_argument('--eval_official', default=True, action='store_true')
...
if __name__ == '__main__':
    ...
    gtfiles = glob.glob(os.path.join(args.groundtruths, '*.txt'))
    print('gt_files', os.listdir(args.groundtruths))
    #tsfiles = [f for f in glob.glob(os.path.join(args.tests, '*.txt')) if not os.path.basename(f).startswith('eval')]
    tsfiles = glob.glob(os.path.join(args.tests, '*.txt'))
    print('ts_files', os.listdir(args.tests))
