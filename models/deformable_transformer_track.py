...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, pre_reference=None, pre_tgt=None, memory=None):
        ...
        bs, _, c = memory.shape#TODO 这种模型集成的做法如何有效？
        if pre_reference is not None:#0.5的概率。相当于先拿这批样本作为测试预测一下（测试时输入的图像略有区别但差不多），得到的预测作为tgt和初始参考点再预测一次。如果网络训练的差不多的话，那么测试的预测应该较为接近，再根据损失微调网络参数。但是两阶段时尽管不用proposal作为tgt，也会继续计算proposal的loss
            ...
            tgt = pre_tgt#preforward的tgt #在轨迹追踪检测时，把上一帧预测的bbox用于这一帧检测时的decoder query
            reference_points = pre_reference#preforward预测的bbox作为初始参考点
            init_reference_out = reference_points

            query_embed = None#注意decoder中的query pos embedding都是None#TODO 为何去掉query embed？是否有效？
            # decoder
            hs, inter_references = self.decoder_track(tgt, reference_points, memory,
                                                      spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten)
            inter_references_out = inter_references
        else:#0.5概率。同deformable detr，从encoder输出中预测最接近GT的500个proposal，并作为decoder输入的初始参考点
            if self.two_stage:
                output_memory, output_proposals = self.gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes)
            ...
            hs, inter_references = self.decoder(tgt, reference_points, memory,
                                                spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten)#decoder本质是在proposal参考点的周围采样encoder信息，
                                                # 然后更新proposal预测的信息。decoder学习的是这种动态采样的机制，从随机生成的tgt（包含坐标信息）预测采样offset，
                                                # 后面每层decoder都有动态的采样。
                                                # 那么这种采样机制到底好不好呢？有可能有的地方的车目标小，确实采样范围小点。
            inter_references_out = inter_references
