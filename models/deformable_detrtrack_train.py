...
class DeformableDETR(nn.Module):
    ...
    def randshift(self, samples, targets):
        ...
        self.xshift = (100 * torch.rand(bs)).int()#bs长度的0到100之间的整数
        self.xshift *= (torch.randn(bs) > 0.0).int() * 2 - 1 #bs长度的随机-1或1序列，使得xshift是负整数或正整数
        ...
        for i, (image, target) in enumerate(zip(samples.tensors, targets)):
            ...
            nopad_image[:,
                  ...
                  max(0, -self.xshift[i]) : min(w, w - self.xshift[i])] #每张图片（无Pad的原始图片）随机裁剪边框
            
            _, patch_h, patch_w = image_patch.shape
            ratio_h, ratio_w = img_h / patch_h,  img_w / patch_w #裁剪前无pad的尺寸相比裁剪后无pad的尺寸的缩小比例
            shifted_image = F.interpolate(image_patch[None], size=(img_h, img_w))[0]#裁剪后的图片重新插值到原始图片（无pad）大小
            pad_shifted_image = copy.deepcopy(image)
            pad_shifted_image[:, :img_h, :img_w] = shifted_image#再把pad加上
            shifted_images.append(pad_shifted_image)
            
            scale = torch.tensor([img_w, img_h, img_w, img_h], device=image.device)[None]
            bboxes = target['boxes'] * scale#像素坐标，宽，高
    ...
    def forward(self, samples_targets, unused_embed=None):
        if self.training:
            ...
            prepre_samples, _ = self.randshift(samples, targets)#随机crop两次
            
            pre_out, pre_embed = self.forward_once(pre_samples, prepre_samples, pre_targets, targets)             
            
            if torch.randn(1).item() > 0.0:#概率0.5，用原图像和一次crop后的图像得到backbone feature（用一次crop和二次crop得到backbone feature预测的结果（输入略有区别，不训练参数）作为proposal和参考点
    ...
    def forward_once(self, samples: NestedTensor, train_samples: NestedTensor, targets=None, next_targets=None):
        ...
        features, pos = self.backbone(samples)#第一次crop后的图像经过backbone处理
        
        if not isinstance(train_samples, NestedTensor):
            train_samples = nested_tensor_from_tensor_list(train_samples)
        pre_feat, _ = self.backbone(train_samples)#第二次crop后的图像经过backbone处理
        
        srcs = []
        masks = []
        
        for l, (feat, feat2) in enumerate(zip(features, pre_feat)):#两次随机crop后分别从backbone得到的特征图
            src, mask = feat.decompose()
            src2, _ = feat2.decompose()
            srcs.append(self.combine(torch.cat([self.input_proj[l](src), self.input_proj[l](src2)], dim=1)))#把backbone输出的两种同层特征图concat后卷积为256维
            masks.append(mask)#两种crop的mask区域是一样的
            assert mask is not None#TODO 这里的两次CROP处理是否有意义
        
        if self.num_feature_levels > len(srcs):
            _len_srcs = len(srcs)
            for l in range(_len_srcs, self.num_feature_levels):
                if l == _len_srcs:
                    src = self.combine(torch.cat([self.input_proj[l](features[-1].tensors), self.input_proj[l](pre_feat[-1].tensors)], dim=1))#level 4
                else:
                    src = self.input_proj[l](srcs[-1])#level 5+
    ...
    def forward_train(self, samples: NestedTensor, pre_embed=None):
        ...
        for l, (feat, feat2) in enumerate(zip(features, pre_feat)):#原始sample从backbone得到的特征图，preforward得到的特征图（随机crop后输入backbone），这样可能对遮挡更鲁棒？
            src, mask = feat.decompose()
            src2, _ = feat2.decompose()
...
#         boxes = torch.gather(boxes, 1, topk_boxes.unsqueeze(-1).repeat(1,1,4))
        ...
        scores, labels = prob[..., 1:2].max(-1)#TODO
        labels = labels + 1
        boxes = box_ops.box_cxcywh_to_xyxy(out_bbox)
