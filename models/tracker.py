...
#                 obj['vxvy'] = [0.0, 0.0]
                ...
                obj['age'] = 1
                ret.append(obj)
                ret_dict[idx] = obj#obj是根据概率阈值筛选后的目标检测结果，包括tracking_id
    ...
    def step(self, output_results):
        ...
        for idx in range(scores.shape[0]):
            if idx in self.tracks_dict and track_bboxes is not None:
                self.tracks_dict[idx]["bbox"] = track_bboxes[idx, :].cpu().numpy().tolist()#上一时刻追踪到的轨迹在这一时刻query的结果（这一时刻可能仍有目标，也可能目标不存在了）

            if scores[idx] >= self.score_thresh:#这一时刻检测到的目标
                obj = dict()
                obj["score"] = float(scores[idx])
                obj["bbox"] = bboxes[idx, :].cpu().numpy().tolist()               
                results.append(obj)        
        ...
        tracks = [v for v in self.tracks_dict.values()] + self.unmatched_tracks#上一时刻追踪的轨迹，和保存的无匹配轨迹
        N = len(results)
        M = len(tracks)
        ...
        if N > 0 and M > 0:
            ...
            matched_indices = linear_sum_assignment(cost_bbox)#上一时刻轨迹在这一时刻追踪的检测框与这一时刻检测框的匹配。若轨迹追踪有目标，应该可以直接匹配到；若轨迹追踪丢了目标，则成为Unmatched，也有可能和这一时刻的新目标匹配到（这种情况应该可以通过IOU损失筛除掉）；也有可能一个unmatched track又找到了当前的检测框
            ...
            for (m0, m1) in zip(matches[0], matches[1]):
                track = results[m0]#若轨迹和检测目标成功匹配，则当前检测目标更新轨迹新的状态
