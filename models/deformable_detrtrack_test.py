...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor, pre_embed=None):
        ...
        if pre_embed is not None:
            pre_feat = pre_embed['feat']#上一时刻的backbone feature
        else:
            pre_feat = features
        ...
        cur_reference = cur_box#前后两帧图像结合预测的bbox
        cur_tgt = cur_hs[-1]
            
        if pre_embed is not None:
            # track mode #上一时刻的track追踪这一时刻位置
            pre_reference, pre_tgt = pre_embed['reference'], pre_embed['tgt']#上上一帧和上一帧结合预测的bbox和tgt
            ...
            outputs_classes = []#把上一帧预测的bbox作为参考点，上一帧的tgt用于这一帧检测时的decoder query
            outputs_coords = []
            for lvl in range(hs.shape[0]):
                if lvl == 0:
                    reference = init_reference
        ...
        out = {'pred_logits': cur_class, 'pred_boxes': cur_box, 'tracking_logits': pre_class, 'tracking_boxes': pre_box}#上一时刻的检测结果在这一时刻图像中query的结果
        
        pre_embed = {'reference': cur_reference, 'tgt': cur_tgt, 'feat': features}#记录这一帧目标检测的结果，会用在下一帧轨迹追踪中
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes):
        ...
        track_topk_values, track_topk_indexes = torch.topk(track_prob.view(track_logits.shape[0], -1), 100, dim=1)
        track_scores = track_topk_values
        track_topk_boxes = track_topk_indexes // out_logits.shape[2]
        track_labels = track_topk_indexes % track_logits.shape[2]
        track_boxes = box_ops.box_cxcywh_to_xyxy(track_bbox)
        track_boxes = torch.gather(track_boxes, 1, track_topk_boxes.unsqueeze(-1).repeat(1,1,4))
        
        #scores, labels = prob[..., 1:2].max(-1)
        #labels = labels + 1#所有target的预测
        #boxes = box_ops.box_cxcywh_to_xyxy(out_bbox)
